...
Shader "Custom/CustomDeferredReflections" {
Properties {
    _SrcBlend ("", Float) = 1
    _DstBlend ("", Float) = 1
}
...
half4 frag (unity_v2f_deferred i) : SV_Target
{
    ...
    half oneMinusReflectivity = 1 - SpecularStrength(schGBuffer.specular);

    half3 worldNormalRefl = reflect(eyeVec, schGBuffer.normalWorld);
    ...
    Unity_GlossyEnvironmentData g = UnityGlossyEnvironmentSetup(1-schGBuffer.roughness, d.worldViewDir, schGBuffer.normalWorld, schGBuffer.specular);

    half3 env0 = UnityGI_IndirectSpecular(d, schGBuffer.occlusion, g);
    ...
    half3 rgb = UNITY_BRDF_PBS (0, schGBuffer.specular, oneMinusReflectivity,1- schGBuffer.roughness, schGBuffer.normalWorld, -eyeVec, light, ind).rgb;
